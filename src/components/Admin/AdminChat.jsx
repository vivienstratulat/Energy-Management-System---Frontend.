...
const AdminChat = () => {
    ...
    const [user, setUser] = useState({ id: "", name: "", email: "", password: "", externalId: "", role: "" });
            ...
            client.connect({}, () => {
                ...
                client.subscribe(`/queue/${user.id}`, message => {
                    ...
                    getPersonById(receivedMessage.fromId).then(person => {
                        toast(`New message: ${receivedMessage.text} from ${person.name}`, { autoClose: 6000 });
                    });
                ...
                client.subscribe(`/queue/typing/${user.id}`, message => {
                   ...
                   /* getPersonById(user.id).then(person => {
                    ...
                    setIsTyping(prev => ({ ...prev, [typingNotification.fromId]: typingNotification.typing }));
                });
    ...
    const updateConversationsWithMessage = (message) => {
        ...
        const conversationId = message.fromId === user.id ? message.toId : message.fromId;
    ...
    const getMessages = async (fromId, toId) => {
        const response = await axios.get(`http://localhost:8091/getMessages/${fromId}/${toId}`);
    ...
    const getMessageById = async (id) => {
        const response = await axios.get(`http://localhost:8091/getMessageById/${id}`);
        return response.data;
    };

    const getPersonById = async (id) => {
        const response = await axios.get(`http://localhost:8080/person/getById/${id}`);
        return response.data;
    };
    ...
    const handleTyping = () => {
        if (stompClient && receiverId) {
            ...
            setTimeout(() => {
                ...
                stompClient.send("/app/typing", {}, JSON.stringify(stopTypingNotification));
            }, 3000);
        }
    };
    ...
    const handleSeen = () => {
        if (stompClient && selectedConversationId) {
            const unseenMessages = conversations[selectedConversationId]?.filter(message => message.toId === user.id && !message.seen);
            if (unseenMessages?.length) {
                const lastUnseenMessage = unseenMessages[unseenMessages.length - 1];
                if (lastUnseenMessage.id !== lastSeenMessageId) {
                    stompClient.send("/app/seen", {}, JSON.stringify(lastUnseenMessage.id));
    ...
    const updateMessageSeenStatus = (messageId) => {
        setConversations(prevConversations => {
            ...
            Object.keys(updatedConversations).forEach(conversationId => {
                updatedConversations[conversationId] = updatedConversations[conversationId].map(msg => 
        ...
        <div className="chat-container">
            <div className="conversation-list">
                {Object.keys(conversations).map(conversationId => (
                    ...
                    >
                        Conversation with {conversationPartners[conversationId] || `User ${conversationId}`}
                    </button>
                ))}
            </div>
    ...
    {isTyping[receiverId] && (
        <div className="typing-notification">
            ...
            {Object.entries(isTyping).map(([userId, typing]) => 
                typing && <div key={userId} className="typing-notification">{conversationPartners[userId] || `User ${userId}`} is typing...</div>
            )}
        </div>
    );
};
